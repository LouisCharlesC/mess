...
namespace mess
{
  template <typename scheduler_type,
            typename value_type,
            std::size_t... predecessors>
  struct kit
  {
    typename kit_customizer<scheduler_type>::template result_type<value_type>
        result;
    typename kit_customizer<scheduler_type>::template latch_type<predecessors...>
        latch;
  };
  ...
  namespace details
  {
    ...
    {
      return std::tuple<
          decltype(make_kit<scheduler_type, flat_graph,
                            details::invoke_result_t<flat_graph, indexes>,
                            indexes>())...>();
    }
    ...
    {
      return []<std::size_t... leaf_nodes_index>(
          std::index_sequence<leaf_nodes_index...>)
      {
        return typename kit_customizer<scheduler_type>::template latch_type<
            leaf_nodes_index...>();
      }
  ...
  using runtime_state =
      decltype(details::make_runtime_state<scheduler_type, flat_graph>(
